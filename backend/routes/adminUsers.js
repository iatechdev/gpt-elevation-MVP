...
router.post('/', async (req, res) => {
  try {
    ...
    const nuevoUsuario = await User.create({ name, email, password: hashedPassword, role, active: true });
...
router.get('/', async (req, res) => {
  try {
    ...
    const usuarios = await User.findAll({
      where,
      attributes: ['id', 'name', 'email', 'role', 'active', 'therapistId', 'planId',
                   'createdAt', 'loginAttempts', 'lockedUntil'],
    ...
    const usuariosConStats = await Promise.all(
      usuarios.map(async (u) => {
        ...
        const moodLogs = await MoodLog.findAll({ where: { UserId: u.id }, attributes: ['checkin_mood', 'checkout_mood'] });
        ...
        // HU-078 — determinar si está bloqueado por intentos fallidos
  ...
  try {
    ...
    const pendingPromptsList = await Promise.all(
      pendingPrompts.map(async (p) => {
        ...
        return { id: p.id, therapistName, version: p.version, proposedBy: p.proposed_by, createdAt: p.createdAt };
      })
    );

    const therapists = await User.findAll({ where: { role: 'therapist', active: true }, attributes: ['id', 'name'] });
    ...
    await Promise.all(therapists.map(async (t) => {
      ...
      if (!profile || !profile.bio || !profile.specialties) therapistsWithoutProfile++;
    }));
...
router.put('/:id', async (req, res) => {
  try {
    ...
    res.json({
      ...
      usuario: { id: usuario.id, name: usuario.name, email: usuario.email, role: usuario.role, active: usuario.active },
    });
  } catch (error) {
...
// ==========================================
// DELETE /api/admin/usuarios/:id — Eliminar usuario (solo superadmin)
// ==========================================
...
router.put('/:id/plan', async (req, res) => {
  try {
    ...
    res.json({
      ...
      usuario: { id: updatedUser.id, name: updatedUser.name, plan: updatedUser.plan ?? null },
    });
  } catch (error) {
...
router.get('/:id/stats', async (req, res) => {
  try {
    ...
    res.json({
      ...
      stats: { sesiones, ratingPromedio, moodPromedio, ultimosRatings: ratings.slice(-5), ultimosMoods: moodLogs.slice(0, 7) },
    });
  } catch (error) {
...
// Desbloquear cuenta — admin y superadmin
// ==========================================
router.put('/:id/unlock', async (req, res) => {
  try {
    ...
    await usuario.update({ loginAttempts: 0, lockedUntil: null, active: true });
...
router.put('/:id/reset-password', async (req, res) => {
  try {
    ...
    await usuario.update({ password: hashedPassword, loginAttempts: 0, lockedUntil: null, active: true });
