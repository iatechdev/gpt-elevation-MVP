...
export function LoginPage() {
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isRegistering) {
        ...
        const checkedToday = localStorage.getItem('elevation_checkin_date') === new Date().toDateString()
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '0 1.5rem', background: '#f9f9f7', fontFamily: 'Inter, sans-serif', position: 'relative' }}>
      <BreathingBackground />
      <div style={{ position: 'relative', zIndex: 1, width: '100%', maxWidth: 360, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
        ...
        <header style={{ textAlign: 'center', marginBottom: '3rem' }}>
          <h1 style={{ fontFamily: 'Playfair Display, serif', fontWeight: 300, letterSpacing: '0.3em', fontSize: '1.5rem', color: '#1C1917', margin: 0 }}>{t('logo')}</h1>
          <div style={{ width: 40, height: 1, background: '#E7E5E4', margin: '1rem auto' }} />
          <p style={{ fontFamily: 'Noto Serif, serif', fontStyle: 'italic', fontSize: '0.8rem', color: '#A8A29E', margin: 0 }}>{t('tagline')}</p>
        </header>
        <form onSubmit={handleAuth} style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: '2rem' }}>
          {isRegistering && (
            <input type="text" value={name} onChange={e => setName(e.target.value)} placeholder={t('label_name')} required
          ...
          <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder={t('label_email')} required
            style={{ width: '100%', background: 'transparent', outline: 'none', padding: '0.75rem 0', borderBottom: '1px solid #E7E5E4', color: '#1C1917', fontSize: '1rem', boxSizing: 'border-box' }} />
          <div style={{ position: 'relative' }}>
            <input type={showPassword ? 'text' : 'password'} value={password} onChange={e => setPassword(e.target.value)} placeholder={t('label_password')} required
          ...
          {authMessage && (
            <p style={{ fontSize: '0.85rem', textAlign: 'center', color: authMessage.startsWith('✓') ? '#0d9488' : '#dc2626', margin: 0 }}>{authMessage}</p>
          )}
          <button type="submit" disabled={isLocked}
            style={{ width: '100%', padding: '0.85rem', color: 'white', fontWeight: 500, borderRadius: '1rem', border: 'none', fontSize: '0.9rem', cursor: isLocked ? 'not-allowed' : 'pointer', background: isLocked ? '#E7E5E4' : 'linear-gradient(135deg,#00685f,#008378)', fontFamily: 'Inter, sans-serif' }}>
            {isLocked ? `🔒 ${t('err_locked')}` : isRegistering ? t('btn_register') : t('btn_login')}
          </button>
          <button type="button" onClick={() => { setIsRegistering(!isRegistering); setAuthMessage(''); setPassword(''); setShowPassword(false) }}
            style={{ background: 'none', border: 'none', color: '#0d9488', fontSize: '0.85rem', cursor: 'pointer', textAlign: 'center', fontFamily: 'Inter, sans-serif' }}>
