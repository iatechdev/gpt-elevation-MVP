...
export function Onboarding() {
    ...
    <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '2rem' }}>
      {Array.from({ length: TOTAL }, (_, i) => (
        <div key={i} style={{
          ...
          height: 8,
          borderRadius: 999,
          background: i + 1 <= step ? '#6B7D5C' : '#E7E5E4',
          transition: 'all 0.2s',
        }} />
      ))}
      <span style={{ fontSize: '0.72rem', color: '#A8A29E', marginLeft: '0.5rem' }}>
        {step}/{TOTAL}
      </span>
    </div>
  )
  ...
  const Title = ({ text }: { text: string }) => (
    <h1 style={{ fontFamily: 'Playfair Display, serif', fontWeight: 400, fontSize: '1.5rem', color: '#1C1917', margin: '0 0 0.5rem' }}>
      {text}
    </h1>
  )

  const Subtitle = ({ text }: { text: string }) => (
    <p style={{ fontSize: '0.875rem', color: '#78716C', margin: '0 0 1.5rem', lineHeight: 1.6 }}>
      {text}
    </p>
  )
    ...
    <div style={containerStyle}>
      <div style={cardStyle}>
        ...
        <Subtitle text={lang === 'es'
          ? 'Un espacio donde la inteligencia artificial, los terapeutas humanos y vos trabajan juntos para construir tu bienestar.'
          : 'A space where AI, human therapists and you work together to build your wellbeing.'} />
  ...
  if (step === 2) return (
    <div style={containerStyle}>
      <div style={cardStyle}>
        ...
        <Subtitle text={lang === 'es'
          ? 'Elegí el tema principal que querés trabajar. Esto personaliza tu experiencia.'
          : 'Choose the main topic you want to work on. This personalizes your experience.'} />
  ...
  if (step === 3) return (
    <div style={containerStyle}>
      <div style={cardStyle}>
        ...
        <Subtitle text={lang === 'es'
          ? 'Cada día tiene tres momentos clave:'
          : 'Each day has three key moments:'} />

        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', marginBottom: '0.5rem' }}>
          {[
            { step: '1', icon: '😊', title: lang === 'es' ? 'Check-in' : 'Check-in', desc: lang === 'es' ? 'Registrás cómo llegás al día' : 'You log how you arrive to the day' },
            { step: '2', icon: '💬', title: lang === 'es' ? 'Conversación' : 'Conversation', desc: lang === 'es' ? 'Hablás con la IA de Elevation' : 'You talk with Elevation AI' },
            { step: '3', icon: '🌙', title: lang === 'es' ? 'Check-out' : 'Check-out', desc: lang === 'es' ? 'Cerrás el día registrando cómo te vas' : 'You close the day logging how you leave' },
          ].map(item => (
            <div key={item.step} style={{ display: 'flex', alignItems: 'flex-start', gap: '0.75rem', padding: '0.85rem', background: '#F5F3EF', borderRadius: '0.65rem' }}>
  ...
  if (step === 4) return (
    <div style={containerStyle}>
      <div style={cardStyle}>
        ...
        <Subtitle text={lang === 'es'
          ? 'Elevation toma tu privacidad muy en serio:'
          : 'Elevation takes your privacy very seriously:'} />
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.65rem', marginBottom: '1.25rem' }}>
          {[
            ...
            <div key={i} style={{ fontSize: '0.875rem', color: '#44403C', padding: '0.6rem 0.85rem', background: '#F5F3EF', borderRadius: '0.65rem' }}>
              {item}
            </div>
          ))}
        </div>
        ...
        <label style={{ display: 'flex', alignItems: 'flex-start', gap: '0.75rem', cursor: 'pointer', marginBottom: '0.5rem' }}>
          ...
          <span style={{ fontSize: '0.82rem', color: '#78716C', lineHeight: 1.5 }}>
            {lang === 'es'
              ? 'Entiendo la política de privacidad y acepto los términos de uso de Elevation.'
              : 'I understand the privacy policy and accept Elevation\'s terms of use.'}
          </span>
        </label>
  ...
  if (step === 5) return (
    <div style={containerStyle}>
      <div style={cardStyle}>
        ...
        <Subtitle text={lang === 'es'
          ? 'Podés explorar Elevation solo con la IA, o conectarte con un terapeuta certificado.'
          : 'You can explore Elevation with AI only, or connect with a certified therapist.'} />

        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
          <button
            onClick={() => { setWantsMatch(true); setStep(6) }}
            style={{
              padding: '1rem',
              border: `1.5px solid ${wantsMatch === true ? '#6B7D5C' : '#E7E5E4'}`,
              background: wantsMatch === true ? '#EAF0E6' : '#fff',
              borderRadius: '0.85rem',
              textAlign: 'left',
              cursor: 'pointer',
              fontFamily: 'Inter, sans-serif',
          ...
          >
            <div style={{ fontSize: '0.95rem', fontWeight: 600, color: '#1C1917' }}>
              🧑‍⚕️ {lang === 'es' ? 'Sí, quiero encontrar mi terapeuta' : 'Yes, I want to find my therapist'}
            </div>
            <div style={{ fontSize: '0.78rem', color: '#78716C', marginTop: '0.25rem' }}>
              {lang === 'es' ? 'Te ayudamos a encontrar el ideal para vos' : 'We\'ll help you find the right one for you'}
            </div>
          </button>

          <button
            onClick={() => { setWantsMatch(false); setStep(6) }}
            style={{
              padding: '1rem',
              border: `1.5px solid ${wantsMatch === false ? '#6B7D5C' : '#E7E5E4'}`,
              background: wantsMatch === false ? '#EAF0E6' : '#fff',
              borderRadius: '0.85rem',
              textAlign: 'left',
              cursor: 'pointer',
              fontFamily: 'Inter, sans-serif',
          ...
          >
            <div style={{ fontSize: '0.95rem', fontWeight: 600, color: '#1C1917' }}>
              🤖 {lang === 'es' ? 'Por ahora solo quiero explorar' : 'For now I just want to explore'}
            </div>
            <div style={{ fontSize: '0.78rem', color: '#78716C', marginTop: '0.25rem' }}>
              {lang === 'es' ? 'Podés conectarte con un terapeuta más adelante' : 'You can connect with a therapist later'}
            </div>
          </button>
        </div>
  ...
  return (
    <div style={containerStyle}>
      <div style={{ ...cardStyle, textAlign: 'center' }}>
        ...
        <Subtitle text={lang === 'es'
          ? wantsMatch
            ? 'Podés buscar tu terapeuta desde el dashboard cuando quieras. Empecemos con tu primer check-in.'
            : 'Tu espacio de bienestar está listo. Empecemos con tu primer check-in del día.'
          : wantsMatch
            ? 'You can find your therapist from the dashboard anytime. Let\'s start with your first check-in.'
            : 'Your wellness space is ready. Let\'s start with your first check-in of the day.'} />
        ...
        <div style={{ background: '#F5F3EF', borderRadius: '0.85rem', padding: '1rem', marginBottom: '0.5rem' }}>
          <p style={{ fontFamily: 'Noto Serif, serif', fontStyle: 'italic', fontSize: '0.95rem', color: '#1C1917', lineHeight: 1.7, margin: 0 }}>
            {lang === 'es'
              ? '"El bienestar es un camino, no un destino. Cada día cuenta."'
              : '"Wellbeing is a journey, not a destination. Every day counts."'}
          </p>
        </div>
        ...
        >
          {saving
            ...
            : (lang === 'es' ? '✓ Empezar mi primer check-in' : '✓ Start my first check-in')}
        </button>
      </div>
    </div>
  )
